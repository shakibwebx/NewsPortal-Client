...
interface ChuadangaCategoryLayoutProps {
  category: Category;
  news: News[];
}

export default function ChuadangaCategoryLayout({ category, news }: ChuadangaCategoryLayoutProps) {
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        {featuredNews && (
          <Link
            ...
            className="col-span-1 md:col-span-2 row-span-1 md:row-span-2 lg:row-span-2 group relative overflow-hidden rounded h-[400px] md:h-auto lg:h-full"
          >
            <Image
        ...
        {regularNews.slice(0, 12).map((newsItem) => (
          <Link
            key={newsItem._id}
            href={`/news/${newsItem.slug || newsItem._id}`}
            className="col-span-1 row-span-1 group relative overflow-hidden rounded hover:shadow-lg transition h-[250px] md:h-auto lg:h-full"
          >
            <Image
              src={newsItem.image || ''}
              alt={newsItem.title}
